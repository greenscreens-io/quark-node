...
const QuarkQueue = require('./Queue');
 ...
 */
class QuarkSocketChannel extends QuarkEvent {
	...
	async #prepareBinaryMessage(message) {
		...
		try {
			...
			console.error(`Failed to prepare binary message: ${error.message}`);
			throw error;
		}
	}
	...
	async #prepareTextMessage(message) {
		...
		try {
			...
			console.error(`Failed to prepare text message: ${error.message}`);
			throw error;
		}
...
module.exports = QuarkSocketChannel;
